...
include("conn.php");
$d = "SELECT kodePro,namaPro FROM cetak_pro";
...
while($r=mysqli_fetch_array($sqlD)){
	$kodeProD = $r['kodePro'];
	$namaProD = $r['namaPro'];
	$url 	= 'https://mfdonline.bps.go.id/master/cetak/cetak_kab.php?idprov='.$kodeProD;
	//var_dump($url);exit;
	echo "Data Ke-".$nod."<br>";
	...
	if (!file_exists($filename)) {
		...
		if(isset($table)){
			...
			foreach($table->find('tr') as $tr){
				...
				//var_dump($no);exit;
				if($no != NULL){
					$kodeKab 	= trim($tr->find('td', 0)->plaintext); 
					$namaKab 	= trim($tr->find('td', 1)->plaintext); 
					...
					$gr = "REPLACE INTO
						...
						(
							kode,
							kodePro,kodeKab,namaPro,namaKab,
							url,
							created_at
						)
						VALUES
					...
					if($ig){
						echo"=> Data ".$nod.'.'.$dtK." - ".$kodePro." - ".$namaPro.'/'.$kodeKab." - ".$namaKab.'<br>';
					}else{
						echo"Failed - ".$url."<br>";
						echo"Failed - ".$gr;exit;
					}
